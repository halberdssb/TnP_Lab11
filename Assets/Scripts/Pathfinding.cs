...
public class Pathfinding : MonoBehaviour
{
    ...
    [Space, SerializeField] private Vector2Int gridCellToMakeObstacle;
    ...
    public void GenerateRandomGrid(int width, int height, float inObstacleProbability)
    {
        ...
        for (int i = 0; i < width; i++)
        {
            ...
            {
                // don't place obstacles on start/end positions
                Vector2Int pos = new Vector2Int(i, j);
                if (pos != start && pos != goal)
                {
                    float randomObstacleSeed = Random.value;
                    bool placeObstacle = randomObstacleSeed <= inObstacleProbability;
    ...
    [ContextMenu("Add Obstacle To Grid")]
    public void AddObstacleToGrid()
    {
        AddObstacle(gridCellToMakeObstacle);
        FindPath(start, goal);
    }
